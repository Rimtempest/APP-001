...
def api_pay_before_exit():
    ...
    # Only session owner or admin can pay
    if session.get('role') != 'admin' and dict(sess)['username'] != session['user']:
        conn.close()
        return jsonify({'ok': False, 'msg': 'You can only pay for your own parking session.'})
...
def api_exit():
    ...
    if not sess:
        conn.close()
        return jsonify({'ok': False, 'msg': 'Invalid or already closed session'})
    # Only session owner or admin can process exit
    if session.get('role') != 'admin' and dict(sess)['username'] != session['user']:
        conn.close()
        return jsonify({'ok': False, 'msg': 'You can only process exit for your own parking session.'})
...
def api_get_profile():
    conn = get_db()
    user = conn.execute("SELECT username,name,email,role,discount_type,id_card_filename FROM users WHERE username=?", (session['user'],)).fetchone()
    conn.close()
    if not user:
        return jsonify({'ok': False, 'msg': 'User not found'})
    return jsonify({'ok': True, 'user': dict(user)})

@app.route('/api/profile/discount', methods=['POST'])
@login_required
def api_update_discount():
    import base64 as b64
    d = request.json
    disc_type = d.get('discount_type', 'none')
    id_card_data = d.get('id_card_data', '')  # base64 image
    id_card_filename = d.get('id_card_filename', '')

    if disc_type not in ['none', 'senior', 'pwd']:
        return jsonify({'ok': False, 'msg': 'Invalid discount type'})

    if disc_type != 'none' and not id_card_data:
        return jsonify({'ok': False, 'msg': 'Please upload your ID card to avail discount'})

    # Save card image if provided
    saved_filename = ''
    if id_card_data and disc_type != 'none':
        os.makedirs('data/id_cards', exist_ok=True)
        ext = id_card_filename.rsplit('.', 1)[-1] if '.' in id_card_filename else 'jpg'
        safe_ext = ext.lower() if ext.lower() in ['jpg','jpeg','png','gif','webp'] else 'jpg'
        saved_filename = f"data/id_cards/{session['user']}_{disc_type}.{safe_ext}"
        try:
            img_bytes = b64.b64decode(id_card_data.split(',')[-1])
            with open(saved_filename, 'wb') as imgf:
                imgf.write(img_bytes)
        except Exception as e:
            return jsonify({'ok': False, 'msg': f'Failed to save ID card: {str(e)}'})

    conn = get_db()
    conn.execute("UPDATE users SET discount_type=?, id_card_filename=? WHERE username=?",
                 (disc_type, saved_filename, session['user']))
    conn.commit()
    conn.close()
    label = {'none': 'None', 'senior': 'Senior Citizen', 'pwd': 'PWD'}[disc_type]
    add_notification(session['user'], f'Discount updated: {label}. Card saved successfully.', 'success')
    return jsonify({'ok': True, 'discount_type': disc_type})
